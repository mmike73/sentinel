...
\begin{itemize}
    ...
    \item \textbf{RF2}: Calcularea unui scor de risc compus pentru fiecare eveniment detectat, fundamentat pe cel puțin un strat de reguli euristice statice.
    ...
    \item \textbf{RF6}: Expunerea unui API REST pentru integrarea opțională a unui modul de Machine Learning (ML), capabil de reantrenare dinamică fără întreruperea monitorizării.
\end{itemize}

\par Cerințele nefuncționale critice pentru un mediu de producție sunt:

\begin{itemize}
    \item \textbf{RN1}: Impactul operațional (overhead-ul) introdus de instrumentarea eBPF asupra procesorului (CPU) să fie limitat sub \textbf{2\%} într-un scenariu de activitate normală a sistemului.
    \item \textbf{RN2}: Latența de reacție --- măsurată de la momentul intercepției până la aplicarea efectivă a măsurii de carantinare (excluzând evaluarea ML) --- să nu depășească \textbf{5 ms}.
    \item \textbf{RN3}: Compatibilitatea arhitecturală necesită un kernel Linux $\geq 5.8$, având subsistemul \textit{cgroup v2} activat.
    \item \textbf{RN4}: Monitorizarea și raportarea continuă a eventualelor pierderi de evenimente cauzate de saturarea memoriei partajate (ring buffer).
\end{itemize}
...
\par Componenta eBPF este proiectată să asigure intercepția a 21 de apeluri de sistem critice. Fiecare apel este instrumentat printr-o sondă kprobe dedicată, având prefixul \texttt{kp\_}. Arhitectura folosește două tipologii principale de rutine de tratare (handlers):
...
\subsection{Stratul 2: Analiza comportamentală a activității în rafală (Burst Detection)}
\label{subsec:burst_detection}

\par Atacurile de tip forță brută (SSH brute-force) sau epuizare a resurselor (fork bombs) se manifestă prin frecvențe anormale de apeluri identice. Stratul 2 implementează o fereastră temporală glisantă (sliding window) care monitorizează agregarea pe tripletul (\texttt{PID}, \texttt{syscall\_nr}, \texttt{start\_time}). Includerea timpului de inițializare a procesului (\texttt{start\_time}) previne coliziunile cauzate de fenomenul de \textit{PID wrap-around} în sistemele cu rotație rapidă a proceselor.
...
\subsection{Stratul 3: Analiza contextuală bazată pe căile fișierelor}
\label{subsec:path_alerts}

\par Al treilea strat realizează o potrivire de tipare (pattern matching) pe căile fișierelor accesate de syscall-urile relevante (\texttt{write} cu fd rezolvat, \texttt{openat}, \texttt{unlinkat}, \texttt{renameat2}), vizând tactici cunoscute de post-exploatare:
...
\begin{itemize}
    \item \texttt{events}: jurnalizează totalitatea evenimentelor relevante. Câmpurile includ \texttt{alert\_type}, \texttt{score}, \texttt{state}, \texttt{pid}, \texttt{comm}, \texttt{timestamp} (ISO 8601) și un payload flexibil de metadate \texttt{detail} (în format JSON, conținând câmpurile \texttt{ts\_ns}, \texttt{syscall\_name}, \texttt{fname} și \texttt{source}). Un flag specific denumit \texttt{source} delimitează acțiunile interceptate direct în kernel (\texttt{"kprobe"}) de pachetele sintetice injectate în mediile de testare (\texttt{"synthetic"}).
    \item \texttt{quarantine\_log}: o pistă de audit irefutabilă pentru măsurile de remediere automată. Leagă acțiunea punitivă de declanșatorul inițial prin cheia externă \texttt{event\_id} și înregistrează metadatele sancțiunii: acțiunile sistemice (\texttt{actions} codificat JSON), confirmarea succesului, erorile OS și durata temporală de execuție a pedepsei (\texttt{latency\_ms}).
\end{itemize}
