...
\par Capitolul de față descrie implementarea concretă a componentelor Sentinel și prezintă rezultatele validării cu suita de 21 de scenarii de atac. Fiecare componentă este tratată separat, cu accent pe deciziile de implementare netriviale și pe aspectele de corectitudine care nu reies direct din proiect.


\section{Mediul de dezvoltare și testare}
\label{sec:dev_env}

\par Dezvoltarea a fost realizată pe un host Ubuntu 22.04 (kernel 6.17.0). Testarea se desfășoară într-o mașină virtuală Vagrant cu Debian 12 (kernel 6.1.0-amd64, cgroup v2 activat implicit), izolând atacurile de mediul de dezvoltare. Mediul virtualizat este esențial: scenariile REP-17 și REP-18 instalează module de kernel reale, iar REP-01 implică un rootkit tip Diamorphine — niciuna dintre acestea nu poate fi rulată în siguranță pe sistemul host.

\par Stiva de unelte utilizată:

\begin{itemize}
    \item \textbf{clang 14+} cu ținta \texttt{bpf} pentru compilarea programului eBPF; flag-ul \texttt{-D\_\_TARGET\_ARCH\_x86} selectează macro-urile corecte de acces la registre.
    \item \textbf{Go 1.21+} cu biblioteca \texttt{github.com/cilium/ebpf} \cite{CiliumEBPF} pentru daemonul și managerul de carantinare.
    \item \textbf{Python 3.12} cu scikit-learn \cite{Pedregosa2011}, FastAPI și uvicorn pentru scorerul ML.
    \item \textbf{SQLite 3.40+} în modul WAL pentru persistența datelor.
...
\par Flag-ul \texttt{-O2} este esențial: verificatorul eBPF impune o limită de 1 milion de instrucțiuni per execuție și respinge programe cu structuri de control prea complexe. Optimizarea la nivel \texttt{O2} reduce numărul de instrucțiuni generate de clang și elimină bucle redundante, facilitând trecerea verificatorului.

\par O decizie de implementare importantă privește extragerea căilor de fișiere. Handler-ul \texttt{submit\_event\_with\_path} utilizează secvența:
...
\begin{enumerate}
    ...
    \item Blochează thread-ul OS cu \texttt{runtime.LockOSThread()} pentru a asigura că atașarea kprobes și operațiunile eBPF rulează pe același OS thread — cerință impusă de specificațiile bibliotecii \texttt{cilium/ebpf} \cite{CiliumEBPF}.
    \item Încarcă colecția eBPF din fișierul \texttt{.bpf.o}, atașează kprobe-urile și deschide reader-ul de ring buffer.
    \item Inserează propriul PID în \texttt{pid\_blacklist}.
\end{enumerate}
...
\par Incluzând \texttt{start\_time} (câmpul 22 din \texttt{/proc/\{pid\}/stat}, în ticks de ceas) în cheie, sistemul distinge între procese diferite care rulează cu același PID pe parcursul testelor — o situație frecventă în suita de atacuri, unde procesele au durată scurtă de viață. Faptul că se folosesc doar ultimele 16 biți ale \texttt{start\_time} introduce o probabilitate mică de coliziune ($\approx 1/65536$), acceptabilă pentru o detecție statistică.
...
\par Scorerul ML (\texttt{scorer/isolation\_forest.py}, 195 de linii) expune un API FastAPI cu trei endpoint-uri: \texttt{GET /health}, \texttt{POST /score} și \texttt{POST /model/train}. Modelul este încărcat lazy la primul apel \texttt{/score}, eliminând latența de inițializare la pornirea serviciului.

\par Vectorizarea evenimentelor produce un vector de 26 de caracteristici. Primele 21 constituie un encoding one-hot al syscall-ului (câte un bit per syscall monitorizat, conform indexului \texttt{SYSCALL\_INDEX}). Caracteristicile suplimentare sunt:

\begin{itemize}
    \item $v_{21}$: flag binar $\mathbb{1}[uid = 0]$ — capturează comportamentul proceselor root.
    \item $v_{22}$: greutatea de risc normalizată $w_{nr}/100$ — injectează cunoașterea a priori a riscului syscall-ului.
    ...
    \item $v_{25}$: $\log_{10}(\text{pid})/6$ — diferențiază procesele de sistem (PID mic) de procesele utilizator.
\end{itemize}

\par Incluziunea cunoașterii a priori a riscului ($v_{22}$) este o alegere deliberată: modelul Isolation Forest este nesupervizicat și nu știe că \texttt{finit\_module} este intrinsec mai periculos decât \texttt{openat}. Caracteristica $v_{22}$ transferă această cunoaștere în spațiul de caracteristici, ghidând modelul fără a-l transforma într-un clasificator supervizat.

\par Modelul Isolation Forest este antrenat pe evenimentele cu scor static $< 30$ din baza de date (care reprezintă comportamentul de bază), cu parametrii $n\_estimators=200$ și $contamination=0.01$. Vectorii sunt standardizați prin \texttt{StandardScaler} \cite{Pedregosa2011} înaintea antrenării și scalarea este aplicată identic la inferență, utilizând parametrii din faza de antrenament. Modelul antrenat este persistat prin \texttt{joblib} și poate fi reantrenant la cald prin \texttt{POST /model/train} fără a opri sau reporni serviciul.

\par Conversia scorului brut IF (valoarea \texttt{decision\_function}, negativă pentru anomalii, pozitivă pentru puncte normale) în scor de amenințare pe scala 0--100 utilizează formula~\ref{eq:sigmoid_transform} din Capitolul~\ref{chap:ch2}. Factorul 25 din exponent produce o separare suficient de abruptă: un punct cu \texttt{raw\_score} $= -0.1$ (ușor anormal) obține \texttt{threat\_score} $\approx 92$, în timp ce un punct cu \texttt{raw\_score} $= 0.1$ (ușor normal) obține \texttt{threat\_score} $\approx 8$.


\section{Implementarea managerului de carantinare}
\label{sec:quarantine_impl}

\par Managerul de carantinare (\texttt{quarantine/main.go}, 306 de linii) validează la pornire disponibilitatea cgroup v2 prin verificarea magic number-ului sistemului de fișiere \texttt{/sys/fs/cgroup} (\texttt{0x63677270} = ``cgrp'' în ASCII) și prezența controllerelor \texttt{cpu} și \texttt{memory} în \texttt{/sys/fs/cgroup/cgroup.controllers}. Această verificare este esențială: pe sisteme cu cgroup v1 sau cgroup v2 fără controllerele activat, acțiunile de carantinare ar eșua silențios.

\par Acțiunile de izolare sunt implementate astfel:

\begin{itemize}
    \item \textbf{THROTTLE}: Creează cgroup \texttt{/sys/fs/cgroup/sentinel/quarantine\_\{pid\}} prin \texttt{os.MkdirAll}, scrie PID-ul procesului în \texttt{cgroup.procs} și scrie \texttt{"10000 100000"} în \texttt{cpu.max}. Această configurare permite procesului să utilizeze 10000 microsecunde din fiecare perioadă de 100000 microsecunde, echivalent cu 10\% CPU.
    \item \textbf{ISOLATE}: Creează cgroup, scrie limita de memorie de $256 \times 2^{20}$ octeți în \texttt{memory.max}, apoi aplică regulile nftables. Valoarea de 256 MiB a fost aleasă ca suficientă pentru a permite terminarea normală a procesului, dar insuficientă pentru exfiltrarea unor volume mari de date în memorie.
    \item \textbf{FREEZE}: Trimite \texttt{SIGSTOP} procesului (oprire imediată, indiferent de starea actuală), creează cgroup-ul și scrie \texttt{"1"} în \texttt{cgroup.freeze}. Combinația SIGSTOP + cgroup.freeze asigură că procesul nu poate fi repornit din exterior cât timp rămâne în cgroup-ul înghețat.
\end{itemize}

\par Izolarea de rețea pentru starea ISOLATE utilizează inodeul cgroup-ului ca identificator de filtrare în nftables. Inodul se obține prin \texttt{syscall.Stat} pe calea cgroup-ului; nftables suportă filtrarea \texttt{meta cgroup} prin inod în kernel $\geq 5.10$. Această abordare evită necesitatea de namespaces de rețea, care ar require modificarea structurii procesului și nu poate fi aplicată unui proces deja în execuție fără cooperarea sa.
...
\par Orchestratorul Python \texttt{run\_sentinel.py} automatizează ciclul de testare:

\begin{enumerate}
    \item Resetare baza de date (DELETE FROM events; PRAGMA wal\_checkpoint).
...
\par Setul de bază cuprinde 11 scenarii, prezentate în Tabelul~\ref{tab:basic_attacks}. Scenariile sunt alese să acopere categoriile principale de amenințări identificate de Cozzi et al. \cite{Cozzi2018}.
...
\par REP-01 (rootkit Diamorphine) este implementat ca un modul de kernel inofensiv care nu produce efecte dăunătoare, dar este compilat și instalat prin \texttt{insmod}. Apelul \texttt{init\_module} sau \texttt{finit\_module} produce un scor de 95, declanșând imediat starea FREEZE. Acesta este scenariul cu cel mai scurt timp de detecție din suita de teste: evenimentul apare în ring buffer în milisequnde de la execuția \texttt{insmod}.

\par REP-11 (injecție ptrace) simulează tehnica clasică de injecție de cod într-un proces țintă printr-o serie de apeluri \texttt{ptrace(PTRACE\_ATTACH, ...)}, \texttt{ptrace(PTRACE\_POKETEXT, ...)} și \texttt{ptrace(PTRACE\_DETACH, ...)}. Syscall-ul \texttt{ptrace} are greutatea de risc 75; amplificat cu 1.3 pentru root ($97.5$, clamped la 95), produce starea FREEZE.

\par REP-12 (brute-force SSH) generează conexiuni repetate la portul 22 printr-un program C care ciclează \texttt{socket()} + \texttt{connect()} de câteva sute de ori per secundă. Stratul de burst detection detectează depășirea pragului de 5 apeluri \texttt{connect} în 3 secunde și emite un alert cu scor 75, producând starea THROTTLE.
...
\textbf{ID} & \textbf{Scenariu} & \textbf{Tip alertă} & \textbf{Stare} \\
\hline\hline
REP-13 & Execuție fileless (memfd) & MemoryInjection    & WARN   \\
\hline
...
\par REP-13 (execuție fileless) simulează tehnica executării codului fără a scrie fișiere pe disc: codul este copiat într-o zonă de memorie anonimă creată cu \texttt{mmap(MAP\_ANONYMOUS)}, marcată executabilă cu \texttt{mprotect(PROT\_EXEC)}, și apelată direct. Syscall-urile \texttt{mmap} și \texttt{mprotect} produc scoruri de 40, respectiv 45, rezultând în WARN.

\par REP-19 (rootkit eBPF stil TripleCross) reproduce tehnica publicată de Pat Hogan (2022), în care un program eBPF malițios se atașează la apeluri de sistem pentru a modifica comportamentul proceselor. Implementarea din test utilizează un program eBPF inofensiv (nu produce efecte dăunătoare) dar apelează \texttt{ptrace} pentru a se atașa la un proces țintă, generând scorul de 95 și starea FREEZE.
...
\par Suprasarcina eBPF a fost evaluată comparând utilizarea CPU pe un workload de compilare kernel (\texttt{make -j4 bzImage}) cu și fără daemonul Sentinel activ, în mașina virtuală de test. Rezultatele medii pe 5 rulări sunt prezentate în Tabelul~\ref{tab:perf}.
