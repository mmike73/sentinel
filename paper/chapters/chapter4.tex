...
\begin{itemize}
    ...
    \item $v_{22}$: ponderea de risc normalizată $w_{nr}/100$ — injectează cunoașterea a priori a riscului syscall-ului.
...
\par Modelul Isolation Forest este antrenat pe evenimentele cu scor static $< 30$ din baza de date (care reprezintă comportamentul de bază), cu parametrii $n\_estimators=200$ și $contamination=0.01$. Vectorii sunt standardizați prin \texttt{StandardScaler} \cite{Pedregosa2011} înaintea antrenării și scalarea este aplicată identic la inferență, utilizând parametrii din faza de antrenament. Modelul antrenat este persistat prin \texttt{joblib} și poate fi antrenat cu date actualizate prin \texttt{POST /model/train} fără a opri sau reporni serviciul.
