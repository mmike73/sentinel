...
\par Evaluarea oricărui sistem modern de detectare a intruziunilor necesită raportarea riguroasă la un cadru standardizat de amenințări. În ecosistemul Linux, vectorii de atac au evoluat de la simple scripturi de exploatare la arhitecturi complexe, mapate exhaustiv în cadrul MITRE ATT\&CK \cite{MITRE_ATTCK}. Evaluarea eficacității sistemului Sentinel se fundamentează pe o analiză teoretică a 21 de scenarii de atac care acoperă tactici critice: persistență, escaladarea privilegiilor, evaziune defensivă, acces la credențiale și execuție.
...
\subsection{Accesul la credențiale și comanda-și-control}
\label{subsec:c2}
...
\par \textbf{Exfiltrarea datelor.} Stabilirea canalelor de comandă și control implică adesea instanțierea de \textit{reverse shells}, procedeu în care gazda compromisă inițiază conexiuni TCP ieșitoare (\textit{outbound}) către atacator, ocolind astfel regulile restrictive de firewall pentru traficul intrant. Amprenta comportamentală constă în serii de apeluri \texttt{socket} și \texttt{connect}, urmate de deturnarea fluxurilor de intrare/ieșire prin invocarea unui shell via \texttt{execve} \cite{Cozzi2018}.